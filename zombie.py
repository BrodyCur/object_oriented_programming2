...
class Zombie:
    ...
    def __str__(self):
        return f"Zombie appears!\nSpeed: {self.speed}\nStrength: {self.strength}\nBattle!: {self.encounter()}"

    @classmethod
    def spawn(cls):
        """Spawns a random number of new zombies, based on the plague level adding each one to the horde.  Each zombie gets a random speed."""
    ...
    @classmethod
    def new_day(cls):
        """Represents the events of yet another day of the zombie apocalypse. Every day some zombies die off (phew!), some new ones show up, and sometimes the zombie plague level increases."""
        Zombie.spawn()
        Zombie.some_die_off()
    ...
    def encounter(self):
        ...
        if outrun == True:
            ...
            return 'You died. Oops. :('

    def chase(self):
        """Represents you trying to outrun this particular zombie. Uses `Zombie.max_speed` to generate a random number that represents how fast you manage to run."""
        your_speed = random.randint(1, Zombie.max_speed)
        return your_speed > self.speed

    def fight(self):
        """Represents you trying to fight off this particular zombie. Uses `Zombie.max_strength` to generate a random number that represents how well you can fight it off."""
        your_strength = random.randint(1, Zombie.max_strength)
        if your_strength > self.strength:

    @classmethod
    def increase_plague_level(cls):
        """Represents the increase of the plague level when a new zombie appears"""
